...
class Testold extends React.Component {
  ...
  render() {
    return (
      ...
      >
        {this.temp()}
      {/* <Testo element={this.temp()} rect={this.state.childRect} bound={this.state.containerRect} /> */}
      </div>
    );
  }
}
...
export default class Test extends React.Component {
  ...
  defaultRect = { x: 0, y: 0, width: 0, height: 0, top: 0, right: 0, bottom: 0, left: 0 };
  ...
  state = {
    ...
    containerRect:  { x: 0, y: 0, width: 0, height: 0, top: 0, right: 0, bottom: 0, left: 0 },
    childRect:  { x: 0, y: 0, width: 0, height: 0, top: 0, right: 0, bottom: 0, left: 0 }
  };
  ...
  handleMouseMove = ({ clientX, clientY }) => {
    ...
    if(this.state.childRect.top + clientY - this.state.originalY <= this.state.containerRect.top) {
      console.log("collision top");
      actualTransY = 0;
    } else if(this.state.childRect.bottom + clientY - this.state.originalY >= this.state.containerRect.bottom) {
      // console.log("collision bottom");
      actualTransY = this.state.containerRect.bottom - this.state.childRect.height;
    }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          {/* {this.props.element} */}
          <div id="rectangle" style={{backgroundColor: "pink", width:'100%', height:'20px'}}
          ref={this.childRef} onMouseDown={this.handleChildMouseDown} />
      </Container>
      </div>
    );
